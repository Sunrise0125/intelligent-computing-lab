...
import torch
import cv2 #opencv 计算机视觉算法，此处的目的是吗，对压缩图像进行解码imdecode
import numpy
import time

#继承自dataset
#定义一个数据集类，用于读取数据和标签，必须实现两个函数__len__:返回数据集长度，__getitem__:返回第idx个样本
class COCODataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #原始图像的二进制流，从 zip 文件中读取，被压缩的数据，尚未解码
        image = self.zip_files.read(file_path)
        #bytes → np.ndarray（（1D uint8 数组））  不过仍然尚未解码成图像，为 OpenCV 的 cv2.imdecode() 做准备，它要求输入为 np.ndarray
...
if __name__ == '__main__':
    # TODO: 使用cpu生成图像转换网络模型（TransNet的实例）并保存在g_net中
    ...
    batch_size = 1
    #DataLoader用于将 Dataset 中的数据 打包成批次（batch）进行迭代，并支持多线程加载、打乱等。
