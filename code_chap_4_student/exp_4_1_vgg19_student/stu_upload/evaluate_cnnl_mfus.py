...
def vgg19():
    ...
    return layer_container    #注意与layer_container（x)的区别，前者不执行只是返回模型，后者执行返回的是，执行模型之后产生的结果张量
...
if __name__ == '__main__':
    ...
    net = vgg19()   #net 的类型是nn.sequential，继承自nn.moudule同时继承了load_state_dict eval（设置成推理模式）等方法
    # TODO: 加载网络参数到net中
...
#     PyTorch 原生是基于动态图的框架。
# 也就是说：模型的计算图是在运行过程中动态构建的，每一次 forward() 调用都会重新创建计算图。
# 在不使用 Python 的环境中运行（比如部署到 C++ 或边缘设备）时 ，需要将动态图转化为静态图，
# 静态图是一种在运行前就将整个前向传播路径转换为固定图结构的方式，代表 ：PyTorch 的 TorchScript 模型。
