...
import time
from PIL import Image  #简单快速图像操作
...
def vgg19():
    ...
    for i, layer_name in enumerate(layers):
        if layer_name.startswith('conv'):
            ...
            # TODO: 在时序容器中执行全连接层计算
            out_features = 4096
            layer_container.add_module(layer_name,nn.Linear(25088,out_features))
            ...
            layer_container.add_module(layer_name,nn.Linear(4096,out_features))
...
def load_image(path):
    ...
    transform = transforms.Compose([transforms.Resize(256),#将图像的最短边缩放到 256 像素，保持原始长宽比例，方便后续的中心裁剪
                                  transforms.CenterCrop(224),#从图像中心裁剪一个 224×224 的正方形区域。
                                  transforms.ToTensor(),
                                  transforms.Normalize(mean=[0.485, 0.456, 0.406],#对每个通道进行标准化处理
                                                      std=[0.229, 0.224, 0.225])])
    #TODO: 对图像调用transform函数进行预处理
    image  = transform(image)

    #TODO: 对tensor的第0维进行扩展，意思就是增加一个第0维，224,224,3 -> 1,224,224,3
