...
# local_rank 值	含义	典型职责
# -1	单机训练（非分布式）	独立处理所有任务
# 0	分布式训练的主进程	数据预处理、缓存、日志记录、同步
# 其他正整数	分布式训练的从属进程	等待主进程完成，复用缓存结果
...
from transformers import (
    ...
    WEIGHTS_NAME,
    AdamW,
    AutoConfig,#自动加载模型配置类
    AutoModelForQuestionAnswering,#自动根据模型名称加载相应的问答模型
    AutoTokenizer,#自动加载与模型匹配的分词器
    get_linear_schedule_with_warmup,
    squad_convert_examples_to_features,#将 SQuAD 格式的数据（SquadExample）转换成模型可以接受的输入特征（Features）
)
...
def train(args, train_dataset, model, tokenizer):
    ...
    #TODO:例化pytorch采样器对训练进行随机采样，分布式则分布式取样
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    #TODO:调用 Pytorch 的 DataLoader 类，构造可迭代对象 train_dataloader
    train_dataloader = DataLoader(train_dataset,sample = train_sampler,batch_size = args.train_batch_size)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        ...
        # 这里应该添加torch.distributed.barrier()等待主进程完成
