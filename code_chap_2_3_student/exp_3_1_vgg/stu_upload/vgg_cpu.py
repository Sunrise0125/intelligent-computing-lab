...
class VGG19(object):
    ...
    def load_image(self, image_dir):
        ...
        # TODO：调整图片维度顺序
        self.input_image = np.transpose(self.input_image,[0,3,1,2])  #还是[]？
    ...
    def forward(self):   # TODO：神经网络的前向传播
        ...
        for idx in range(len(self.param_layer_name)):
            ...
            current = self.layers[self.param_layer_name[idx]].forward(current)  # 调用每一层的forward方法
