...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784, 
        ...
        self.net.setInputShape(batch_size, input_size, 1, 1) #设置输入参数  dim1 dim2 dim3 dim4
        # fc1
        ...
        input_shapem2[1] = 1
        input_shapem2[2] = 1
        input_shapem2[3] = hidden1  # fc2的输入是fc1的输出

        weight_shapem2 = pycnnl.IntVector(4)
        weight_shapem2[0] = batch_size
        weight_shapem2[1] = 1
        weight_shapem2[2] = hidden1  # 连接fc1的输出
        weight_shapem2[3] = hidden2  # fc2的输出维度

        output_shapem2 = pycnnl.IntVector(4)
        output_shapem2[0] = batch_size
        output_shapem2[1] = 1
        output_shapem2[2] = 1
        output_shapem2[3] = hidden2  # fc2的最终输出
        ...
        input_shapem3[1] = 1
        input_shapem3[2] = 1
        input_shapem3[3] = hidden2  # fc3的输入是fc2的输出

        weight_shapem3 = pycnnl.IntVector(4)
        weight_shapem3[0] = batch_size
        weight_shapem3[1] = 1
        weight_shapem3[2] = hidden2  # 连接fc2的输出
        weight_shapem3[3] = out_classes  # 最终输出类别数

        output_shapem3 = pycnnl.IntVector(4)
        output_shapem3[0] = batch_size
        output_shapem3[1] = 1
        output_shapem3[2] = 1
        output_shapem3[3] = out_classes  # 最终的分类输出


        self.net.createMlpLayer('fc3', input_shapem3, weight_shapem3, output_shapem3)

        input_shape_s = pycnnl.IntVector(3)
        input_shape_s[0] = batch_size
        input_shape_s[1] = 1
        input_shape_s[2] = out_classes
        self.net.createSoftmaxLayer('softmax',input_shape_s,1)  #
