...
class AdamOptimizer(object):
    def __init__(self, lr, diff_shape):
        ...
        self.eps = 1e-8
        self.lr = lr  # 学习率
        self.mt = np.zeros(diff_shape)  # 一阶动量（梯度均值）
        self.vt = np.zeros(diff_shape)  # 二阶动量（梯度方差）
        self.step = 0  # 迭代步数
    ...
    def update(self, input, grad):
        ...
        # 偏差修正（应对初始零偏差）
        mt_hat = self.mt / (1 - self.beta1 ** self.step)  # 修正后的一阶动量
        vt_hat = self.vt / (1 - self.beta2 ** self.step)  # 修正后的二阶动量
        
        # 参数更新：输入 - 学习率 * 动量调整后的梯度
