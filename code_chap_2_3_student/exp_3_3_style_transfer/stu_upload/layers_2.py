...
class ConvolutionalLayer(object):
            ...
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        region = self.input_pad[idxn, :, h_start:h_end, w_start:w_end].flatten()  # [channel_in, kernel_size, kernel_size]
                        
                        # 计算卷积（点积 + 偏置）
                        self.output[idxn, idxc, idxh, idxw] = np.dot(region,self.weight[:,:,:,idxc].flatten()) + self.bias[idxc]      
    ...
    def backward_raw(self, top_diff):
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        ...
                        self.d_weight[:, :, :, idxc] += top_diff[idxn,idxc,idxh,idxw] * self.input_pad[idxn,:,self.stride*idxh:self.stride*idxh+self.kernel_size,idxw*self.stride:idxw*self.stride+self.kernel_size]
        ...
        bottom_diff = bottom_diff[:, :, self.padding: self.input_pad.shape[2] - self.padding, self.padding: self.input_pad.shape[3] - self.padding]   #裁剪掉扩充的边界四周的self.padding
        self.backward_time = time.time() - start_time
        return bottom_diff
...
class MaxPoolingLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        #这里创建了 max_elements 矩阵，它是一个 one-hot 编码矩阵，用于记录最大值的位置
